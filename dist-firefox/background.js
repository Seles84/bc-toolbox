...
function isApiRequest(data) {
  return typeof data === "object" && data !== null && data.kind === "bct-api";
}
...
class BctDatabase extends Dexie {
  members;
  chat;
  chatChannels;
  memberSeen;
...
const db = new BctDatabase();
const ADDON_META_PREFIX = "";
function parseBeepMessage(raw) {
  ...
  if (raw.includes(ADDON_META_PREFIX)) {
    ...
    const meta = metaLine ? tryParse(metaLine.slice(1)) : void 0;
    if (meta) {
      const text = lines.filter((line) => !line.startsWith(ADDON_META_PREFIX)).join("\n").trim();
      return { text: text || void 0, meta };
    }
  }
...
const CHAT_TYPES = ["Chat", "Emote", "Activity", "Whisper", "Action", "ServerMessage"];
const tabs = /* @__PURE__ */ new Map();
const STORAGE_PREFIX = "tabState:";
async function getTab(tabId) {
  let state = tabs.get(tabId);
  if (!state) {
    state = { tabId, port: null, modLoaded: false, needsRefresh: false, pendingQueries: /* @__PURE__ */ new Map() };
    tabs.set(tabId, state);
    const stored = await chrome.storage.session.get(STORAGE_PREFIX + tabId);
    const persisted = stored[STORAGE_PREFIX + tabId];
    if (persisted) {
...
async function persistTab(state) {
  const persisted = {
...
function findTabByMember(memberNumber) {
...
function queryPage(state, query, timeoutMs = 25e3) {
  return new Promise((resolve) => {
    ...
    const message = { kind: "query", id, query };
    try {
...
const STORED_MESSAGE_TYPES = new Set(CHAT_TYPES);
let alertKeywords = [];
void chrome.storage.local.get("alertKeywords").then((stored) => {
  alertKeywords = stored.alertKeywords ?? [];
});
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === "local" && changes.alertKeywords) {
    alertKeywords = changes.alertKeywords.newValue ?? [];
  }
});
const KEYWORD_COOLDOWN = 6e4;
const keywordNotifiedAt = /* @__PURE__ */ new Map();
async function maybeKeywordAlert(state, line, senderName) {
...
async function handlePageMessage(state, message) {
  switch (message.kind) {
    case "mod-loaded":
      state.modLoaded = true;
      state.needsRefresh = Boolean(message.stale) || message.build !== "92494c2a66ef";
...
async function storeCapturedProfile(profile) {
  await upsertMember(profileToRecord(profile, Date.now()));
}
async function onLogin(state, player) {
  const now = Date.now();
...
async function closeDanglingSessions(member, timestamp) {
  const open = await db.playerSessions.where("[member+ended]").equals([member, 0]).toArray();
  for (const session of open) {
    await db.chatChannels.where("sessionId").equals(session.sessionId).and((channel) => channel.left === 0).modify({ left: timestamp });
    if (session.id !== void 0) {
      await db.playerSessions.update(session.id, { ended: timestamp });
    }
  }
}
async function onLogout(state) {
  const now = Date.now();
  if (state.channelId) {
  ...
  state.memberNumber = void 0;
  state.characterName = void 0;
  state.sessionId = void 0;
  state.channelId = void 0;
  state.roomName = void 0;
  await persistTab(state);
}
async function onGameEvent(state, direction, event, data, timestamp) {
  if (direction === "server") {
    switch (event) {
      case "ChatRoomSync":
        return onRoomSync(state, data, timestamp);
      case "ChatRoomSyncMemberJoin":
        return onMemberJoin(state, data, timestamp);
      case "ChatRoomSyncMemberLeave":
        return onMemberLeave(state, data, timestamp);
...
async function onBeep(state, direction, data, timestamp) {
  const beep = data;
  ...
  let memberName = direction === "in" ? beep.MemberName : void 0;
  if (!memberName) {
    const record = await db.members.get(beep.MemberNumber);
    memberName = record ? record.nickname || record.name : void 0;
  }
  ...
  await db.beeps.add({
    ...
    message: text,
    metadata: meta,
    roomName: direction === "in" ? beep.ChatRoomName ?? void 0 : void 0,
    created: timestamp
  });
}
async function onRoomSync(state, data, timestamp) {
  if (!data?.Name) {
    return;
  }
  if (!state.sessionId) {
  ...
  const memberNumbers = (data.Character ?? []).map((c) => c.MemberNumber).filter((n) => typeof n === "number");
  if (state.channelId && state.roomName === data.Name) {
    const channel = await db.chatChannels.get(state.channelId);
    const merged = [.../* @__PURE__ */ new Set([...channel?.memberNumbers ?? [], ...memberNumbers])];
...
async function onMemberJoin(state, data, timestamp) {
  if (!data?.Character) {
    return;
  }
...
async function onMemberLeave(state, data, timestamp) {
  if (typeof data?.SourceMemberNumber === "number") {
    await markSeen(state, data.SourceMemberNumber, timestamp);
  }
}
const NOTIFY_COOLDOWN = 30 * 6e4;
const lastNotified = /* @__PURE__ */ new Map();
async function onAccountQueryResult(state, data, timestamp) {
  const result = data;
  ...
  for (const friend of state.friends) {
    ...
    const note = await db.notes.where("[member+viewer]").equals([friend.memberNumber, state.memberNumber]).first();
    ...
    chrome.notifications.create(`bct-friend:${state.memberNumber}:${friend.memberNumber}`, {
      ...
      message: friend.private ? "In a private room" : friend.chatRoomName ? `In ${friend.chatRoomName}` : "Not in a room yet"
    });
  }
}
async function onChatLine(state, line, timestamp) {
  ...
  if (!senderName) {
    ...
    senderName = sender ? sender.nickname || sender.name : void 0;
  }
  let target = line.target;
  if (line.type === "Whisper" && target === void 0 && state.memberNumber && line.sender !== state.memberNumber) {
...
async function onRoomLeave(state, timestamp) {
  if (state.channelId) {
    await db.chatChannels.update(state.channelId, { left: timestamp });
  }
  state.channelId = void 0;
...
function profileToRecord(profile, timestamp) {
...
async function captureRoomCharacter(state, bundle, timestamp) {
...
async function upsertMember(record) {
  ...
  if (existing) {
    ...
    const nicknameChanged = defined.nickname !== void 0 && !!existing.nickname && defined.nickname !== existing.nickname;
    ...
    const ownershipChanged = "ownership" in defined && existing.ownership !== void 0 && ownershipKey(defined.ownership) !== ownershipKey(existing.ownership);
    const lovershipChanged = defined.lovership !== void 0 && existing.lovership !== void 0 && lovershipKey(defined.lovership) !== lovershipKey(existing.lovership);
...
function ownershipKey(ownership) {
  return ownership?.MemberNumber !== void 0 ? `${ownership.MemberNumber}:${ownership.Stage ?? 0}` : "none";
}
function lovershipKey(lovership) {
  return (lovership ?? []).filter((l) => l.MemberNumber !== void 0 || l.Name).map((l) => `${l.MemberNumber ?? l.Name}:${l.Stage ?? 0}`).sort().join("|");
}
async function markSeen(state, member, timestamp) {
...
const BACKUP_DEFAULTS = { enabled: false, intervalDays: 7, keep: 4 };
async function getBackupSettings() {
  const stored = await chrome.storage.local.get("autoBackup");
  return { ...BACKUP_DEFAULTS, ...stored.autoBackup ?? {} };
}
  ...
  void getTab(tabId).then((state) => {
    ...
    port.onMessage.addListener((message) => {
      void getTab(tabId).then(
        (s) => handlePageMessage(s, message).catch((error) => {
...
const handlers = {
  async ping() {
    return { now: Date.now(), version: "1.1.1" };
  },
  async "tabs.status"() {
  ...
  async "page.query"({ memberNumber, query }) {
    ...
    if (result.success && query.type === "character-data") {
      await storeCapturedProfile(result.data).catch(
...
async function dispatch(request) {
  try {
    const handler = handlers[request.endpoint];
    if (!handler) {
